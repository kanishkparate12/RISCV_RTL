...
The processor core does not contain instruction memory or data memory internally. Instead, it interfaces with external memories through dedicated instruction and data memory ports.

| Component | Purpose | Access |
|---|---|---|
| External Instruction Memory | Stores program instructions | Read-only, word-aligned |
| External Data Memory | Stores runtime data | Read/write, word-aligned |
| Register File (`regfile.v`) | CPU working registers (`x0`–`x31`) | 2 asynchronous reads, 1 synchronous write |

The processor communicates with external memories using the following interface:

| Signal | Direction | Description |
|---|---|---|
| `pc_out` | Output | Address used to fetch the next instruction |
| `instr` | Input | Instruction supplied by external instruction memory |
| `dmem_we` | Output | Data memory write enable (`SW`) |
| `dmem_addr` | Output | Data memory address generated by the ALU |
| `dmem_wd` | Output | Data to be written to external data memory |
| `dmem_rd` | Input | Data returned by external data memory (`LW`) |

During simulation, the instruction memory and data memory may be modeled using Verilog `reg` arrays in the testbench or wrapper modules. For FPGA implementations, these interfaces can connect to block RAMs, while ASIC implementations can replace them with SRAM macros or memory bus interfaces.
